...
public class RSSFeedGeneratorImpl implements RSSFeedGenerator {
    ...
    public static void main (String[] args) {
        RSS rss = new RSS();
        Channel chan = new Channel("test","http://localhost:8080/","desc test");
        Item item = new Item("item test","desc item test test");
        chan.addItem(item);
        rss.addChannel(chan);
        try {
            File out = new File("/home/olesiuk/Public/rss.xml");
            new RSSFeedGeneratorImpl().generateToFile(rss,out);
           String rssXml = new RSSFeedGeneratorImpl().generateAsString(rss);
    ...
    /* (non-Javadoc)
     * @see rss.generator.RSSFeedGenerator#generateToFile(viecili.jrss.generator.elem.RSS, java.io.File)
     */
    public void generateToFile(RSS rss, File xmlFile) throws IOException {
        // TODO method generateToFile(RSSFeedGeneratorImpl)
        Document doc = generateDocument(rss);
        OutputFormat format = new OutputFormat(doc);
        format.setLineSeparator(LineSeparator.Windows);
        format.setIndenting(true);
